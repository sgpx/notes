...
void x_qsort(void *v[], int left, int right, 
	...
	for(i = left+1; i <= right; i++)
		if(rv == 0 && (*comp)(v[i], v[left]) < 0)
			swap(v, ++last, i);
		else if(rv == 1 && (*comp)(v[i], v[left]) > 0)
			swap(v, ++last, i);

	swap(v, left, last);
	x_qsort(v, left, last-1, comp, rv);
	x_qsort(v, last+1, right, comp, rv);
}
...
int main(int argc, char *argv[]){
	...
	if( (nlines = readlines(lineptr, MAXLINES)) > 0){
		...
		x_qsort((void**) lineptr, 0, nlines-1, fnx, reverse_order);
