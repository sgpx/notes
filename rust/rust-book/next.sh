#!/bin/bash
engine="gpt4.sh"
echo "I am learning rust by example. I don't know rust very well. this is all the work I have done so far: " > ~/tmp.txt
for a in $(ls arch/u*.rs); do echo $a >> ~/tmp.txt ; echo === >> ~/tmp.txt ; cat $a >> ~/tmp.txt ; echo === >> ~/tmp.txt ;  done
