...
if [ "$1" = "-s" ]; then
	...
	for curr_file in $(ls *.$ext); do chk=$(grep "${curr_file}," $lockfile) ; if [ "$chk" != "" ]; then echo "skipping $curr_file" ; continue ; else echo summarizing $curr_file ; fi ; printf "summarize in 1 line the concept or problem solved here, not the numerical values:\n\n" > $LLM_INPUT_FILE ; cat $curr_file >> $LLM_INPUT_FILE ; echo $(basename $PWD)/${curr_file},$($LLM --noconfirm),$(get_date_string $curr_file) >> $lockfile ; tail -n1 $lockfile ; done
else
